...
impl Map {
    fn path(&self) -> usize {
        ...
        let mut q = VecDeque::from([(self.start, 0usize)]);
        let mut visited = HashSet::from([self.start]);
        let check = |x: i32, y: i32| -> bool { x >= 0 && x < m && y >= 0 && y < n };
        ...
        while let Some((p, d)) = q.pop_front() {
            ...
            for i in 0..4 {
                ...
                if check(nx, ny)
                    ...
                    && (self.map[ny as usize][nx as usize] - self.map[p.1 as usize][p.0 as usize])
                        <= 1
                {
