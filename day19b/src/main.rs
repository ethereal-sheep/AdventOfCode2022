    ...
    fn simulate(&self, n: u64) -> u64 {
        self.blueprints
            ...
            .map(|b| {
                ...
                let mut ans = 0;
                let rr = Factory::dfs(b, &mut memo, &mut ans, 1, n);
                print(rr);
                rr >> 56
            })
            .fold(1, |a, x| a * x)
...
impl Factory {
    fn dfs(
        ...
        if n == 0 {
            *curr_ans = (*curr_ans).max(resources_robots >> 56);
        ...
        // if geodes + (n * geode-bot) + triangle(n) < curr ans { don bother }
        let geodes = resources_robots >> 56;
        let geode_bots = (resources_robots >> 24) & 0b11111111;
        let triangle = (n * (n - 1)) / 2;
        if geodes + (geode_bots * n) + triangle < *curr_ans {
            memo.insert(hash, 0);
            return 0;
        }
...
fn main() {
    ...
    println!("{}", simulation.simulate(32));
}
